...
import org.notabug.lifeuser.ArraysOfCinemas.activity.FilterActivity;
...
public class ListFragment extends BaseFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mShowArrayList = new ArrayList<JSONObject>();
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_import) {
            ...
            databaseHelper.importDatabase(getActivity());
            return true;
        }
    ...
    private void filterAdapter() {
        ...
        switch (sharedPreferences.getString(FilterActivity.FILTER_SORT, null)) {
            case "best_rated":
                if(mSearchView) {
    ...
    private void createShowList() {
        mShowGenreList = new HashMap<String, String>();
        mShowArrayList = getShowsFromDatabase(null, MovieDatabaseHelper.COLUMN_ID + " DESC");
        mShowAdapter = new ShowBaseAdapter(mShowArrayList, mShowGenreList,
                preferences.getBoolean(SHOWS_LIST_PREFERENCE, true));
    ...
    private ArrayList<JSONObject> getShowsFromDatabase(String searchQuery, String order) {
        ...
        if(searchQuery != null && !searchQuery.equals("")) {
            cursor = mDatabase.rawQuery("SELECT * FROM " + mDatabaseHelper.TABLE_MOVIES
                    + " WHERE " + mDatabaseHelper.COLUMN_TITLE + " LIKE '%"
                    + searchQuery + "%'" + dbOrder, null);
        } else {
            cursor = mDatabase.rawQuery("SELECT * FROM " +
                    mDatabaseHelper.TABLE_MOVIES + dbOrder, null);
        }
    ...
    private ArrayList<JSONObject> convertDatabaseListToArrayList(Cursor cursor) {
        ArrayList<JSONObject> dbShowsArrayList = new ArrayList<JSONObject>();
        ...
        while(!cursor.isAfterLast()) {
            ...
            try {
                ...
                showObject.put(mDatabaseHelper.COLUMN_CATEGORIES, cursor.getInt(
                ...
                if(cursor.getInt(cursor.getColumnIndex(mDatabaseHelper.COLUMN_MOVIE)) != 1) {
